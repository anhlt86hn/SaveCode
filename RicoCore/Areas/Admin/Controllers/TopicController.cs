...
namespace RicoCore.Areas.Admin.Controllers
{
    ...
    public class TopicController : BaseController
    {
        ...
        public TopicController(IPostCategoryService postCategoryService, IPostService postService, IAuthorizationService authorizationService, IHostingEnvironment hostingEnvironment, IConfiguration configuration)
        {
        ...
        [Route("admin/topic/{url}")]
        public async Task<IActionResult> Index(string url, string keyword, int? kichcotrang, string sapxep, int trang = 1)
        {
            var result = await _authorizationService.AuthorizeAsync(User, "USER", Operations.Read);
            if (result.Succeeded == false)
                return new RedirectResult("/Admin/Home/Index");

            var posts = new PagedResultPostViewModel();
            if (kichcotrang == null)
                kichcotrang = _configuration.GetValue<int>("PageSize");

            //pageSize = 10;
            posts.PageSize = kichcotrang;
            posts.SortType = sapxep;            
            var postCategory = _postCategoryService.GetByUrl(url);
            var postCategoryId = postCategory.Id;
            posts.CategoryId = postCategoryId;
            posts.Data = _postService.GetPostsByTopic(postCategoryId, keyword, trang, kichcotrang.Value, posts.SortType);
            posts.CategoryName = postCategory.Name;            
            return View(posts);
        }
